...
- Publishing: bump the version, commit, create an annotated tag `v{version}` (use `git tag -a`, not `git tag`; GitHub only lists annotated tags in the Releases UI and `git push tags` won't send a lightweight tag unless named explicitly), then create a GitHub Release on that tag. The `release: published` event triggers `.github/workflows/publish.yml`, which verifies the tag matches the `package.json` version, runs `bun test`, and runs `npm publish --access public` using the `NPM_TOKEN` repo secret.
