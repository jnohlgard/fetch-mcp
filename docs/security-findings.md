...
| # | Severity | Finding | Affected MCP tools |
|---|---|---|---|
| 1 | Critical | SSRF bypass via IPv4-mapped IPv6 (`[::ffff:x.x.x.x]`) | **all six**: `fetch_html`, `fetch_json`, `fetch_txt`, `fetch_markdown`, `fetch_readable`, `fetch_youtube_transcript` |
| 2 | Critical | yt-dlp path bypasses all URL validation (no protocol/IP checks, no size limit) | `fetch_youtube_transcript` (only when `yt-dlp` is on PATH) |
| 3 | Critical | Redirect SSRF is detect-after-the-fact, not prevention | **all six** |
| 4 | High | Argument injection into `yt-dlp` via `lang` (regex allows leading hyphens) | `fetch_youtube_transcript` |
| 5 | High | yt-dlp branch is Unix-only (`mktemp`, `which`) and degrades silently | `fetch_youtube_transcript` |
| 6 | Medium | `extractPlayerResponse` regex truncates at first `};` in JSON | `fetch_youtube_transcript` (direct path) |
| 7 | Medium | `parseTranscriptXml`: attribute-order fragility, silent empty transcript, incomplete entity decoding | `fetch_youtube_transcript` |
| 8 | Medium | Caption URL built by string concatenation (`&fmt=srv1`) | `fetch_youtube_transcript` (direct path) |
| 9 | Low | `json()` parse→stringify round-trip mutates large numbers | `fetch_json` |
| 10 | Low | `applyLengthLimits` can split surrogate pairs via `substring` | **all six** (whenever a limit lands on a surrogate pair) |
| 11 | Low | `hasYtDlp` cached for process lifetime; stale in long-lived server | `fetch_youtube_transcript` |
| 12 | Note | JSDOM must stay without `runScripts` — adding it is RCE | `fetch_txt`, `fetch_markdown`, `fetch_readable` |
...
### 1. SSRF bypass via IPv4-mapped IPv6 — reproduced end-to-end
...
### 2. yt-dlp path bypasses all URL validation
...
### 3. Redirect SSRF is detection-only, not prevention
...
### 4. Argument injection into yt-dlp via `lang`
...
### 5. yt-dlp branch is Unix-only and fails silently
...
### 6. `extractPlayerResponse` truncates at the first `};`
...
### 7. `parseTranscriptXml` fragility and silent empty result
...
### 8. Caption URL built by string concatenation
...
### 9. `json()` round-trip mutates data
...
### 10. `applyLengthLimits` splits surrogate pairs
...
### 11. `hasYtDlp` cached for process lifetime
...
### 12. Note: keep JSDOM script-free
...
Ordered by effort-first with severity kept in view: quick wins that close real attack surface come before contained parsing work, which comes before the one genuinely involved investigation.
